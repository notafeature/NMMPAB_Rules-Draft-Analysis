...
REVISIONS = {
    ...
    "pathways": [
        ("July 28, 2026", "Every starting license, every route, and every step is now written into the page itself, by tools/sync-pathways.py, instead of being built by JavaScript when the page loads. Nothing a reader sees has changed: the profiles, the verdicts, the steps, the status flags, the citations, and the notes on what the committee recommendation would change all read exactly as before. What changed is that a browser's find command now reaches every step, a search engine can index them, and a reader without JavaScript is shown all five starting licenses in turn, each with all of its routes, rather than a pointer to another page."),
