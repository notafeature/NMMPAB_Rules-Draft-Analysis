...
| `research.medical-psilocybin.org` | The Research and Continuous Improvement Committee workgroup's published work product, by revision; signed, so it is its own host (below) | Built September 11, 2026 (repository `research.medical-psilocybin.org`): `/2026-09-11/`, the Core / Extended Data Set as Summary, Schema, workbook, JSON Schema, CSV, and three diagrams, with a files table carrying size and SHA-256. Not counted | A later revision is a new dated folder; a revision is not edited after it is published |
