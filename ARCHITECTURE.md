...
| `research.medical-psilocybin.org` | The Core / Extended Data Set recommended by the Research and Continuous Improvement Committee; signed, so it is its own host (below) | Built September 11, 2026 (repository `research.medical-psilocybin.org`): one page and the files beside it at fixed addresses, Summary, Schema, workbook, JSON Schema, CSV, and three diagrams, with a files table carrying size and SHA-256. Not counted | A later revision replaces the files at the same addresses and moves the previous ones to `archive/<date>/` |
